...
18. **`zod` must stay on 3.x and be deduped via `overrides`**: `@lmstudio/sdk` declares `zod@^3.22.4` and throws on construction under zod 4. Two zod majors in one tree also makes the MCP SDK's `zod-compat` union collapse, yielding TS2589 "excessively deep" errors across `src/mcp/tools/**`. The `overrides.zod` pin keeps exactly one copy — without it, resolution flips nondeterministically between runs.
