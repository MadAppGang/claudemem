...
const session = recorder.finalize(
	[
		{
			criterion: { type: "file_exists", path: "src/components/Button.tsx" },
			passed: true,
		},
		{
			criterion: {
...
console.log(
	`    - Correction rate: ${(session.metrics.correctionRate * 100).toFixed(1)}%`,
);
console.log(
	`    - Error rate: ${(session.metrics.errorRate * 100).toFixed(1)}%`,
);
console.log(
	`    - Autonomy rate: ${(session.metrics.autonomyRate * 100).toFixed(1)}%`,
);
...
const requiredN = stats.calculateRequiredSampleSize({
	alpha: 0.05,
	power: 0.8,
	minEffectSize: 0.1, // 10% improvement
...
const achievedPower = stats.calculateAchievedPower(50, 0.1, 0.25);
console.log(
	`  With n=50, achieved power: ${(achievedPower * 100).toFixed(1)}%`,
);
...
const baselineSessions = Array.from({ length: 30 }, () =>
	createMockSession(0.25 + Math.random() * 0.1, Math.random() > 0.3),
);

// Treatment: 20% correction rate, 80% success (improved)
const treatmentSessions = Array.from({ length: 30 }, () =>
	createMockSession(0.15 + Math.random() * 0.1, Math.random() > 0.2),
);
...
console.log("\n  Correction Rate:");
console.log(
	`    Baseline: ${(comparison.correctionRate.baseline * 100).toFixed(1)}%`,
);
console.log(
	`    Treatment: ${(comparison.correctionRate.treatment * 100).toFixed(1)}%`,
);
console.log(
	`    Change: ${(comparison.correctionRate.relativeChange * 100).toFixed(1)}%`,
);
console.log(`    p-value: ${comparison.correctionRate.pValue.toFixed(4)}`);
console.log(
	`    Significant: ${comparison.correctionRate.statisticallySignificant ? "✅ Yes" : "❌ No"}`,
);
console.log(
	`    Improved: ${comparison.correctionRate.improved ? "✅ Yes" : "❌ No"}`,
);

console.log("\n  Success Rate:");
console.log(
	`    Baseline: ${(comparison.successRate.baseline * 100).toFixed(1)}%`,
);
console.log(
	`    Treatment: ${(comparison.successRate.treatment * 100).toFixed(1)}%`,
);
console.log(
	`    Change: ${(comparison.successRate.relativeChange * 100).toFixed(1)}%`,
);
console.log(`    p-value: ${comparison.successRate.pValue.toFixed(4)}`);
console.log(
	`    Significant: ${comparison.successRate.statisticallySignificant ? "✅ Yes" : "❌ No"}`,
);
...
const calculateAggregates = (sessions: RecordedSession[]) => ({
	...
	successRate:
		sessions.filter((s) => s.outcome === "success").length / sessions.length,
	avgCorrectionRate:
		sessions.reduce((sum, s) => sum + s.metrics.correctionRate, 0) /
		sessions.length,
	avgErrorRate:
		sessions.reduce((sum, s) => sum + s.metrics.errorRate, 0) / sessions.length,
	avgAutonomyRate:
		sessions.reduce((sum, s) => sum + s.metrics.autonomyRate, 0) /
		sessions.length,
	avgDurationMs:
...
if (decision.significantMetrics.length > 0) {
	console.log(
		`  Significant Metrics: ${decision.significantMetrics.join(", ")}`,
	);
}
...
console.log(
	`Persona: ${scenario.persona.expertiseLevel} (${scenario.persona.correctionStyle})`,
);
console.log(`\nInitial Prompt:`);
...
const response = await syntheticAgent.processAgentResponse(
	{
		content:
			"I can help improve the app. What aspect would you like me to focus on?",
		isQuestion: true,
...
console.log(
	`    Criteria passed: ${retrieved?.successCriteria.filter((c) => c.passed).length}/${retrieved?.successCriteria.length}`,
);
...
console.log(
	`  Avg correction rate: ${(summaryStats.avgCorrectionRate * 100).toFixed(1)}%`,
);
