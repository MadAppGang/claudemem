...
export class SyntheticAgent {
	...
	constructor(scenario: ValidationScenario, sessionRecorder: SessionRecorder) {
		this.scenario = scenario;
		this.persona = scenario.persona;
		this.knowledgeBase = scenario.knowledgeBase;
		this.sessionRecorder = sessionRecorder;

		this.queryHandler = new QueryHandler(this.knowledgeBase, this.persona);
	...
	private createAbandonResponse(): SyntheticResponse {
		const messages: Record<string, string> = {
			...
			direct: "Let's stop. Too many issues. I'll try a different approach.",
			frustrated: "Forget it. This is taking too long and not going anywhere.",
		};
...
export class QueryHandler {
	...
	private checkKnowledgeBase(question: string): string | null {
		...
		if (
			...
			if (kb.componentStyle) return `Use ${kb.componentStyle} components`;
		}
	...
	private formatAnswer(content: string, source: string): QueryAnswer {
		let text = content;
