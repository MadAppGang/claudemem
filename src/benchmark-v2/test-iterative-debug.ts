...
async function testIterativeLogic() {
	...
	const summaries = [
		{ modelId: "haiku", summary: "Adds two numbers together and returns the sum" },
		{ modelId: "deepseek", summary: "A function that computes the addition of two numeric values" },
	];
	...
	for (let i = 0; i < summaries.length; i++) {
		...
		const testSimilarity = cosineSimilarity(queryEmbedding, summaryEmbeddings.embeddings[i]);
		console.log(`Test summary similarity: ${testSimilarity.toFixed(4)}`);

		const allSimilarities: Array<{ modelId: string; similarity: number; isTest: boolean }> = [
			{ modelId: testSummary.modelId, similarity: testSimilarity, isTest: true },
		];

		for (let j = 0; j < competitors.length; j++) {
			const compIndex = summaries.findIndex(s => s.modelId === competitors[j].modelId);
			const compSimilarity = cosineSimilarity(queryEmbedding, summaryEmbeddings.embeddings[compIndex]);
			allSimilarities.push({
				modelId: competitors[j].modelId,
				similarity: compSimilarity,
				isTest: false
			});
			console.log(`Competitor ${competitors[j].modelId} similarity: ${compSimilarity.toFixed(4)}`);
		}
		...
		allSimilarities.forEach((s, idx) => {
			...
			console.log(`  #${idx + 1}: ${s.modelId} (${s.similarity.toFixed(4)})${marker}`);
		});
		...
		if (!passed) {
			console.log(`  → Would trigger refinement (rank ${rank} > target ${effectiveTargetRank})`);
		} else {
			console.log(`  → Passes without refinement (rank ${rank} <= target ${effectiveTargetRank})`);
		}
		console.log();
	}
	...
	console.log("If both models' summaries rank #1 when tested against each other,");
	console.log("then both would pass without refinement → 0.0 rnd for both.");
	console.log("\nThis can happen if:");
	console.log("  1. Embeddings are very similar (both summaries semantically equivalent)");
	console.log("  2. Query favors one model consistently but we test each separately");
