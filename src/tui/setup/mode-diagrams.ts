#!/usr/bin/env bun
// Colored deployment mode diagrams for mnemex setup wizard
...
const latRowLoc = (
	...
	o(
		`${pad(name, 9)}${pad(where, 7)}${pad(lat(ms), 9)}${gauge(ms, maxMs)}  ${note}`,
	);
};
...
const netLine = (label: string, ms: number) => {
	const latStr = lat(ms);
	const bar = gray(
		"┄".repeat(W - strip(label).length - strip(latStr).length - 4),
	);
	o(`${bar} ${label} ${latStr}`);
};
...
const modelPanel = (
	...
	o(`${gray("┌───────────┐")}`);
	oT(
		`${gray("│")} ${fillColor(fill)} ${gray("│")}  ${step(stepName)}  ${gray(desc)}`,
		ms,
	);
...
function mode1(showTable = false) {
	...
	rDiv();
	o(
		`${green("●")} Runtime   ${white("Ollama / LM Studio")} ${gray("— 100% local")}`,
	);
	o(
		`${green("●")} Privacy   ${bgreen(bold("AIR-GAPPED"))} ${gray("zero bytes leave machine")}`,
	);
	o(
		`${green("●")} Requires  ${white("Apple Silicon / NVIDIA GPU")}  ${byellow("≥16 GB")}`,
	);
...
function mode2(showTable = false) {
	...
	o("");
	oT(
		`  ${cyan("═══════")} ${bgGreen(" vectors → cloud ")} ${cyan("═══════")}`,
		80,
	);
	...
	o(`${gray("┌───────────┐")}`);
	oT(
		`${gray("│")} ${magenta("▓▓▓▓▓▓▓▓▓")} ${gray("│")}  ${step("[rerank]")}  ${gray("cloud")}`,
		80,
	);
	o(
		`${gray("│")} ${pad(magenta("Claude"), 9)} ${gray("│")}  ${gray("Claude Sonnet ~175B")}`,
	);
	o(
		`${gray("│")} ${pad(magenta("Sonnet"), 9)} ${gray("│")}  ${gray("H100 server-side")}`,
	);
	o(`${gray("└─────┬─────┘")}`);
	o("");
	oT(
		`  ${cyan("═══════")} ${bgBlue(" results ← cloud ")} ${cyan("═══════")}`,
		80,
	);
	o("");
	oT(`  ${cyan("▼")}  ${step("[enrich]")}  ${gray("code summaries")}`, 500);
	...
	o(
		`${green("●")} Privacy   ${bgreen(bold("CODE NEVER LEAVES"))} ${gray("only vectors")}`,
	);
	o(
		`${green("●")} Requires  ${white("Apple Silicon / NVIDIA GPU")}  ${byellow("≥5 GB")}`,
	);
...
function mode3(showTable = false) {
	...
	o(`${gray("┌───────────┐")}`);
	oT(
		`${gray("│")} ${magenta("▓▓▓▓░░░░░")} ${gray("│")}  ${step("[embed]")}  ${gray("cloud API")}`,
		20,
	);
	o(
		`${gray("│")} ${pad(magenta("OpenAI"), 9)} ${gray("│")}  ${gray("text-embedding-3-large")}`,
	);
	o(
		`${gray("│")} ${pad(magenta("cluster"), 9)} ${gray("│")}  ${gray("~1-2B params, 3072-dim")}`,
	);
	o(`${gray("└─────┬─────┘")}`);
	o(`  ${cyan("│")}`);
	...
	o(
		`${gray("│")} ${pad(magenta("Sonnet"), 9)} ${gray("│")}  ${gray("Anthropic H100 cluster")}`,
	);
	o(`${gray("└─────┬─────┘")}`);
...
function comparison() {
	...
	rTop("COMPARISON");
	o(
		`${pad("", 14)}${pad(bgGreen("Local"), 13)}${pad(bgGreen("Team"), 13)}${bgBlue("Cloud")}`,
	);
	sep();
	c(
		"Embed model",
...
export async function selectMode(): Promise<"local" | "shared" | "full-cloud"> {
	...
	const render = () => {
		...
		console.log();
		console.log(
			`${gray("┌─")} ${bold("mnemex")} ${gray("─ Setup ─ Step 1 ─ Choose Deployment Mode ─┐")}`,
		);
...
if (import.meta.main) {
	...
	console.log(
		bold("  ║          MNEMEX DEPLOYMENT MODE DIAGRAMS             ║"),
	);
	console.log(
		bold("  ╚══════════════════════════════════════════════════════════╝"),
	);
