...
const CHUNK_TYPE_ABBREV: Record<string, string> = {
	function: "fn", method: "method", class: "cls", interface: "iface",
	type: "type", enum: "enum", module: "mod", block: "block",
	"document-section": "doc", chunk: "code", file: "file",
};

const KIND_COLORS: Record<string, string> = {
	function: "#61AFEF", method: "#61AFEF",
	class: "#E5C07B", interface: "#E5C07B", type: "#E5C07B", enum: "#E5C07B",
	module: "#8B5CF6",
};
 ...
 *   File: path\n\nSummary: text\n\nResponsibilities:\n- item\n\nExports: a, b
 */
function parseSummaryContent(raw: string, maxW: number): Array<{ label?: string; lines: string[] }> {
	const parts: Array<{ label?: string; lines: string[] }> = [];

	// Split on known section labels at start of line
	const sectionLabels = ["File", "Summary", "Responsibilities", "Exports", "Dependencies", "Patterns", "Parameters", "Returns", "Side effects", "Usage", "Signature"];
...
function formatPR(pr: number, symbolCount = 1500): string {
	const avg = 1 / symbolCount;
	const ratio = pr / avg;
	const pctStr = pr >= 0.01 ? `${(pr * 100).toFixed(1)}%` : `${(pr * 100).toFixed(2)}%`;
...
function parsePartInfo(name?: string): { partIndex: number; totalParts: number } | null {
	if (!name) return null;
...
function SymbolRow({ sym, index, symbolCount }: { sym: SymbolDefinition; index: number; symbolCount: number }) {
	...
	return (
		<box height={1} flexDirection="row">
			...
			<box><text fg={theme.text}>{sym.name.length > 28 ? `${sym.name.slice(0, 26)}\u2026` : sym.name}</text></box>
			<box><text fg={theme.dimmed}>{"  "}</text></box>
			<box><text fg={theme.muted}>{loc}</text></box>
			<box><text fg={theme.dimmed}>{"  "}</text></box>
			<box backgroundColor={prBadgeBg(pr, symbolCount)}>
				<text fg="#FFFFFF">{` PR:${prLabel} `}</text>
			</box>
			{sym.isExported && (
				<box><text fg={theme.success}>{" exported"}</text></box>
			)}
		</box>
	);
}
...
function InfoRow({ label, value, valueFg }: { label: string; value: string; valueFg?: string }) {
	return (
		<box height={1} flexDirection="row">
			<box><text fg={theme.muted}>{`    ${label.padEnd(12)}`}</text></box>
			<box><text fg={valueFg ?? theme.text}>{value}</text></box>
		</box>
	);
}
...
export function ResultDetailView({ result, allResults, onClose }: ResultDetailViewProps) {
	const { tracker, projectPath } = useAppContext();
	const { width } = useTerminalDimensions();

	// Local override for navigating to parts not in search results
	const [overrideResult, setOverrideResult] = useState<SearchResult | null>(null);
		...
		async function loadSiblings() {
			try {
				...
				for (const r of allResults) {
					...
					if (rBase === symbolName && rPi && r.chunk.filePath === chunk.filePath) {
	...
	const name = chunk.name
		? chunk.parentName ? `${chunk.parentName}.${chunk.name}` : chunk.name
		: "unnamed";
	...
	const hasSymbolSummary = !!effectiveSummary && effectiveSummary !== activeResult.fileSummary;
	const hasFileSummary = !!activeResult.fileSummary;

	function parseSummaryToLines(raw: string): Array<{ text: string; fg: string; indent: number }> {
	...
	const isDeadCode = graphLoaded && symbolName && callers.length === 0 && focusedSymbol !== null;
	...
	return (
		<box flexDirection="column" width="100%" height="100%">
			{/* ── Title bar ────────────────────────────────────────────────── */}
			<box height={1} width="100%" backgroundColor="#1E3A5F" flexDirection="row">
				<box><text fg={theme.text}>{`  ${chunk.filePath}:${chunk.startLine}-${chunk.endLine}  `}</text></box>
				<box backgroundColor={kindColor}>
					<text fg="#000000">{` ${badge} `}</text>
				</box>
				<box><text fg={theme.dimmed}>{" "}</text></box>
				<box><text fg={theme.text}>{name}</text></box>
				<box><text fg={theme.dimmed}>{"  "}</text></box>
			...
			{/* ── Part navigation bar ─────────────────────────────────────── */}
			{partInfo && allSiblingResults.length > 1 && (() => {
				const hasLeft = currentSiblingIdx > 0;
				const hasRight = currentSiblingIdx >= 0 && currentSiblingIdx < allSiblingResults.length - 1;
				return (
					<box height={1} width="100%" backgroundColor="#37474F" flexDirection="row">
						<box><text fg={hasLeft ? theme.info : theme.dimmed}>{`  ${hasLeft ? "\u25C0" : " "} `}</text></box>
						<box><text fg={theme.text}>{`Part ${partInfo.partIndex}/${partInfo.totalParts}`}</text></box>
			...
			<scrollbox width="100%" height="100%">
				...
				<InfoRow label="language" value={chunk.language || "unknown"} valueFg={theme.info} />
				<InfoRow label="lines" value={`${chunk.startLine}-${chunk.endLine} (${lineCount} lines)`} />
				<InfoRow label="pagerank" value={prStr} valueFg={getScoreColor(normalizePR(focusedSymbol?.pagerankScore ?? 0, symbolCount))} />
				{focusedSymbol?.isExported && (
					<InfoRow label="exported" value="yes" valueFg={theme.success} />
				)}
				{isDeadCode && (
					<InfoRow label="status" value="POTENTIALLY DEAD (0 callers)" valueFg="#EF5350" />
				)}
				{sig && (
					<box height={1} flexDirection="row">
						<box><text fg={theme.muted}>{"    sig         "}</text></box>
						<box><text fg={theme.warning}>{sig.length > width - 20 ? `${sig.slice(0, width - 23)}\u2026` : sig}</text></box>
					</box>
				)}

				{/* ── Dead code warning ────────────────────────────────── */}
				{isDeadCode && (
					<box height={1} width="100%" backgroundColor="#B71C1C" flexDirection="row">
						<box><text fg="#FFFFFF">{`  POTENTIALLY DEAD CODE  `}</text></box>
						<box><text fg="#FFCDD2">{`0 callers found — this symbol may be unused`}</text></box>
					</box>
				)}
				...
				{hasSymbolSummary && (
					<>
						<box height={1}><text fg={theme.dimmed}>{""}</text></box>
						<SectionHeader title="SYMBOL SUMMARY" bg="#37474F" />
						{symbolSummaryLines.map((sl, i) => (
							<box key={`ss-${i}`} height={1} paddingLeft={sl.indent} flexDirection="row">
				...
				{hasFileSummary && (
					<>
						...
						{fileSummaryLines.map((sl, i) => (
							<box key={`fs-${i}`} height={1} paddingLeft={sl.indent} flexDirection="row">
								<text fg={sl.fg}>{sl.text}</text>
							</box>
						))}
					</>
				...
				{!hasSummary && (
					<>
						...
						<box height={1} flexDirection="row" paddingLeft={4}>
							<text fg={theme.dimmed}>{"no summary (run claudemem index to generate)"}</text>
						</box>
					</>
				)}
				...
				{!graphLoaded && (
					<box height={1} flexDirection="row">
						<box><text fg={theme.dimmed}>{"    loading graph..."}</text></box>
					</box>
				)}
				{graphLoaded && callers.length === 0 && (
					<box height={1} flexDirection="row">
						<box><text fg={theme.dimmed}>{"    no callers found"}</text></box>
					</box>
				)}
				{callers.map((sym, i) => (
				...
				{graphLoaded && callees.length === 0 && (
					<box height={1} flexDirection="row">
						<box><text fg={theme.dimmed}>{"    no callees found"}</text></box>
					</box>
				)}
				{callees.map((sym, i) => (
				...
				{codeLines.map((line, i) => {
					...
					return (
						<box key={`code-${lineNo}`} height={1} flexDirection="row">
							<box><text fg={theme.dimmed}>{` ${num} \u2502 `}</text></box>
							<box><SyntaxLine line={line} lang={lang} /></box>
						</box>
					);
				})}
			</scrollbox>
