...
/**
 * Find the mnemex binary path.
 *
 * Resolution order:
 * 1. VS Code setting mnemex.binaryPath (if non-empty)
 * 2. `which mnemex` (user's shell PATH via /bin/sh)
 * 3. Common install locations (bun global, npm global, /usr/local/bin)
 */
export function findMnemex(): string {
	...
	try {
		const result = execSync('/bin/sh -c "which mnemex"', {
			encoding: "utf8",
			timeout: 3000,
		});
	...
	const candidates = [
		path.join(home, ".bun", "install", "global", "bin", "mnemex"),
		path.join(home, ".npm-global", "bin", "mnemex"),
		path.join(home, ".local", "bin", "mnemex"),
		"/usr/local/bin/mnemex",
		"/usr/bin/mnemex",
		"/opt/homebrew/bin/mnemex",
	];
...
/**
 * Detect whether the mnemex binary needs to run under bun.
...
function findBun(): string {
	try {
		const result = execSync('/bin/sh -c "which bun"', {
			encoding: "utf8",
			timeout: 3000,
		});
		const found = result.trim();
...
/**
 * CliBridge: spawns the mnemex CLI with --agent mode, collects output, and
...
export class CliBridge {
	...
	constructor() {
		...
		log(
			`CliBridge: binary=${this._binaryPath} useBun=${this._useBun}${this._useBun ? ` bunPath=${this._bunPath}` : ""} timeout=${this._timeoutMs}ms`,
		);
	}
	...
	private _spawnArgs(args: string[]): { command: string; spawnArgs: string[] } {
		if (this._useBun) {
			return {
				command: this._bunPath,
				spawnArgs: ["run", this._binaryPath, ...args, "--agent"],
			};
		}
	...
	/**
	 * Run a mnemex command and collect all stdout. Rejects on non-zero exit,
			...
			return await new Promise<string>((resolve, reject) => {
				...
				child.stdout.on("data", (chunk: Buffer) => {
					if (stdoutChunks.length === 0) {
						log(
							`  first stdout at +${Date.now() - spawnTime}ms (${chunk.length} bytes)`,
						);
					}
				...
				child.stderr.on("data", (chunk: Buffer) => {
					log(
						`  stderr at +${Date.now() - spawnTime}ms: ${chunk.toString("utf8").trim().slice(0, 200)}`,
					);
				...
				child.on("error", (err: NodeJS.ErrnoException) => {
					if (err.name === "AbortError" || controller.signal.aborted) {
						...
						const hint = this._useBun
							? `bun not found at "${this._bunPath}". Install bun or set mnemex.binaryPath.`
							: `mnemex binary not found at "${this._binaryPath}". Install mnemex or set mnemex.binaryPath in settings.`;
						reject(new CliBridgeError(hint, "", null));
					} else {
			...
			return await new Promise<void>((resolve, reject) => {
				...
				child.on("error", (err: NodeJS.ErrnoException) => {
					if (err.name === "AbortError" || controller.signal.aborted) {
						reject(
							new CliBridgeError(
								"Index command timed out or was cancelled",
								"",
								null,
							),
						);
				...
				child.on("close", (code: number | null) => {
					...
					if (code !== 0) {
						const msg =
							stderr.trim() || `Index command exited with code ${code}`;
						reject(new CliBridgeError(msg, stderr, code));
					} else {
						resolve();
					}
