...
describe("parsePrefix", () => {
	...
	it("handles caller prefix with multiple fields", () => {
		const result = parsePrefix(
			"caller name=myFunc file=src/a.ts line=5 kind=function",
		);
	...
	it("handles symbol prefix with rank field", () => {
		const result = parsePrefix(
			"symbol name=MyClass kind=class line=1 rank=0.12",
		);
	...
	it("handles file path with spaces in prefix line", () => {
		const result = parsePrefix(
			"caller name=login file=/Users/John Doe/project/src/auth.ts line=42 kind=function",
		);
		expect(result.prefix).toBe("caller");
