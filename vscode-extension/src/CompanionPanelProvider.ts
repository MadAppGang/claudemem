...
export class CompanionPanelProvider {
	...
	/**
	 * Open or reveal the companion panel. Called by the mnemex.openCompanion command.
	 */
	open(): void {
		log("Companion: open()");
		if (this._panel) {
	...
	private async _fetchSymbolData(symbolName: string): Promise<void> {
		...
		const callers = ctx
			? {
					symbol: ctx.symbol,
					callers: ctx.callers.map((c) => ({ ...c, kind: c.kind ?? "" })),
				}
			: null;
		const callees = ctx
			? {
					symbol: ctx.symbol,
					callees: ctx.callees.map((c) => ({ ...c, kind: c.kind ?? "" })),
				}
			: null;
		...
		if (symbol) {
			try {
				...
				log(
					`Companion: failed to read source: ${err instanceof Error ? err.message : String(err)}`,
				);
			}
		}
		...
		if (seq === this._fetchSeq) {
			try {
				const searchResults = await this._bridge.search(
					symbolName,
					projectPath,
				);
				if (seq === this._fetchSeq) {
					const match = searchResults.find(
						(r) => r.name === symbolName && r.summary,
					);
					if (match?.summary) {
						this._postMessage({
							type: "companionUpdate",
...
<head>
  ...
  <title>mnemex: Symbol Context</title>
</head>
<body>
