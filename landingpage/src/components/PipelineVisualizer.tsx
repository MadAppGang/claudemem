...
export const PipelineVisualizer: React.FC = () => {
    ...
    const indexingSteps = [
        { 
            id: '1', title: 'File Discovery', type: 'LOCAL', visual: 'files',
            time: '< 1s', cost: 'Free',
            technical: 'Walks directory tree. Respects .gitignore. Filters for source extensions.',
            value: 'Ensures only relevant source code is indexed, skipping node_modules/build artifacts.'
        },
        { 
            id: '2', title: 'AST Parsing', type: 'LOCAL', visual: 'ast',
            time: '~5s', cost: 'Free',
            technical: 'Parses files into Abstract Syntax Trees using Tree-sitter.',
            value: 'Understands code structure (functions, classes) rather than just reading raw text.'
        },
        { 
            id: '3', title: 'Semantic Chunking', type: 'LOCAL', visual: 'chunks',
            time: '~2s', cost: 'Free',
            technical: 'Splits code at function/class boundaries (300-500 tokens).',
            value: 'Keeps related logic together. Naive splitters cut functions in half; we don\'t.'
        },
        { 
            id: '4', title: 'Symbol Graph', type: 'LOCAL', visual: 'graph',
            time: '~3s', cost: 'Free',
            technical: 'Builds a graph of definitions, references, and calls.',
            value: 'Maps the "nervous system" of your code to understand dependencies.'
        },
        { 
            id: '5', title: 'PageRank Scoring', type: 'LOCAL', visual: 'pagerank',
            time: '< 1s', cost: 'Free',
            technical: 'Calculates eigen-centrality for every symbol.',
            value: 'Identifies critical infrastructure (high rank) vs isolated utilities (low rank).'
        },
        { 
            id: '6', title: 'Summary Generation', type: 'AI', visual: 'ai-summary',
            time: '2-10m', cost: 'Requires LLM',
            technical: 'LLM generates intent-focused summaries for each chunk.',
            value: 'Captures "intent" ("validates user") vs "implementation" ("regex check").'
        },
        { 
            id: '7', title: 'Embedding Gen', type: 'AI', visual: 'embedding',
            time: '1-5m', cost: 'Requires Model',
            technical: 'Converts code + summary into high-dimensional vectors.',
            value: 'Enables semantic search (finding concepts) rather than just keyword matching.'
        },
        { 
            id: '8', title: 'Local Storage', type: 'LOCAL', visual: 'storage',
            time: '~2s', cost: 'Free',
    ...
    const retrievalSteps = [
        { 
            id: '1', title: 'Query Embedding', type: 'AI', visual: 'query-vec',
            time: '100ms', cost: 'Negligible',
            technical: 'Converts user query into a vector.',
            value: 'Translates your question into the same mathematical space as your code.'
        },
        { 
            id: '2', title: 'Hybrid Search', type: 'LOCAL', visual: 'hybrid',
            time: '30ms', cost: 'Free',
            technical: 'Runs Vector Search + BM25 Keyword Search in parallel.',
            value: 'Vectors find concepts ("auth"); BM25 finds exact matches ("validateToken").'
        },
        { 
            id: '3', title: 'Score Fusion', type: 'LOCAL', visual: 'fusion',
            time: '5ms', cost: 'Free',
        ...
        { 
            id: '4', title: 'Context Expansion', type: 'LOCAL', visual: 'context',
            time: '20ms', cost: 'Free',
            technical: 'Retrieves imports, types, and callers for top results.',
            value: 'Provides the LLM with the surrounding context needed to understand the code.'
        },
        { 
            id: '5', title: 'Response Assembly', type: 'LOCAL', visual: 'assembly',
            time: '5ms', cost: 'Free',
            technical: 'Formats retrieval results into an optimized XML prompt.',
            value: 'Packages everything into a format Claude/GPT can instantly process.'
        },
        { 
            id: '6', title: 'Delivered', type: 'OUTPUT', visual: 'delivered',
            time: 'Total: ~200ms', cost: 'Free',
    ...
    return (
        <section ref={sectionRef} className="py-32 bg-[#080808] border-t border-white/5 relative overflow-hidden">
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                
                <div className="text-center mb-16">
                    <h2 className="text-4xl md:text-5xl font-black text-white mb-6">See Under the Hood</h2>
                    ...
                    <div className="flex justify-center mt-8">
                        <div className="bg-[#111] p-1 rounded-lg border border-white/10 inline-flex shadow-lg relative z-20">
                            <button 
                                onClick={() => { setMode('indexing'); setActiveStep(0); }}
                            ...
                            <button 
                                onClick={() => { setMode('retrieval'); setActiveStep(0); }}
                            ...
                            {steps.map((step, idx) => (
                                ...
                                >
                                    <div className={`
                                        ...
                                        ${idx === activeStep 
                                            ? step.type === 'AI' ? 'bg-purple-500/20 text-purple-400 border-purple-500/50' : 'bg-claude-ish/20 text-claude-ish border-claude-ish/50'
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between mb-0.5">
                                            <div className={`font-bold text-sm truncate ${idx === activeStep ? 'text-white' : 'text-gray-400'}`}>
                                                {step.title}
                                            </div>
                                            {idx === activeStep && (
                                                <span className={`text-[9px] font-mono px-1.5 py-0.5 rounded ${
                                                    step.type === 'AI' ? 'bg-purple-500/20 text-purple-400' : 
                                                    step.type === 'LOCAL' ? 'bg-gray-800 text-gray-400' : 'bg-green-500/20 text-green-400'
                                                }`}>
                                                    {step.type}
                                                </span>
                                            )}
                        ...
                        <div className="bg-[#0c0c0c] border border-white/10 rounded-2xl shadow-2xl h-full flex flex-col relative overflow-hidden ring-1 ring-white/5">
                            ...
                            <div className="h-12 border-b border-white/10 bg-[#111]/80 backdrop-blur flex items-center justify-between px-6 z-10 shrink-0">
                                <div className="font-mono text-xs text-gray-400 uppercase tracking-widest flex items-center gap-2">
                                    <span className={`w-2 h-2 rounded-full animate-pulse ${steps[activeStep].type === 'AI' ? 'bg-purple-500' : 'bg-green-500'}`}></span>
                            ...
                            <div className="flex-1 relative flex items-center justify-center p-8 overflow-hidden bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.03)_0%,transparent_70%)]">
                                ...
                                <div className="absolute top-6 right-6 flex flex-col gap-2 z-20 items-end pointer-events-none">
                                    <div className="bg-[#0a0a0a]/90 backdrop-blur border border-white/10 px-3 py-2 rounded-lg flex items-center gap-3 shadow-xl">
                                        <div className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Time</div>
                                        <div className="text-xs font-mono text-white">{steps[activeStep].time}</div>
                                    </div>
                                    <div className="bg-[#0a0a0a]/90 backdrop-blur border border-white/10 px-3 py-2 rounded-lg flex items-center gap-3 shadow-xl">
                                        <div className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Cost</div>
                                        <div className={`text-xs font-mono ${steps[activeStep].type === 'AI' ? 'text-purple-400' : 'text-green-400'}`}>
                                            {steps[activeStep].cost}
                                        </div>
                                    </div>
                                </div>

                                <div key={activeStep} className="scale-125 md:scale-150 transform transition-all duration-500">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-white/10">
                                    ...
                                    <div className="p-5 space-y-2">
                                        ...
                                        <p className="text-xs text-gray-400 font-mono leading-relaxed h-8 flex items-center">
                                            ...
                                            <span className="animate-pulse">{steps[activeStep].technical}</span>
                                        </p>
                                    </div>
...
const EngineVisualizer: React.FC<{ visual: string }> = ({ visual }) => {
    switch (visual) {
        case 'files': return <VisualFiles />;
        case 'ast': return <VisualAST />;
        case 'chunks': return <VisualChunking />;
        case 'graph': return <VisualGraphBuilding />;
        case 'pagerank': return <VisualPageRankActive />;
        case 'ai-summary': return <VisualAISummary />;
        case 'embedding': return <VisualEmbedding />;
        case 'storage': return <VisualStorage />;
        
        // Retrieval
        case 'query-vec': return <VisualQueryVec />;
        case 'hybrid': return <VisualHybridSearch />;
        case 'fusion': return <VisualScoreFusion />;
        case 'context': return <VisualContextExpansion />;
        case 'assembly': return <VisualAssembly />;
        case 'delivered': return <VisualDelivered />;
        
        default: return <div className="text-gray-600 font-mono">Processing...</div>;
    }
};
...
const VisualFiles = () => (
    <div className="w-full max-w-sm space-y-2 relative overflow-hidden h-64">
        ...
        <div className="animate-[flow-up_2s_linear_infinite] space-y-2">
            {[...Array(10)].map((_, i) => (
                <div key={i} className="flex items-center gap-3 p-2 border border-white/5 rounded bg-[#111]">
        ...
        <div className="relative w-40 h-40 flex items-center justify-center">
            <svg className="w-full h-full overflow-visible">
                <g className="animate-fadeIn">
                    ...
                    <circle cx="30%" cy="40%" r="3" fill="#3fb950" className="animate-pulse" />
    ...
    <div className="w-64 bg-[#111] border border-white/10 rounded p-4 font-mono text-[10px] text-gray-500 relative overflow-hidden">
        <div className="absolute inset-0 bg-transparent z-20 flex flex-col">
            ...
            <div className="flex-1 border-2 border-claude-ish/30 bg-claude-ish/5 m-1 rounded flex items-center justify-center relative">
                <span className="bg-black text-claude-ish px-1 rounded absolute top-0 right-0">Chunk 1</span>
            </div>
            {/* Chunk 2 Overlay */}
            <div className="flex-1 border-2 border-blue-500/30 bg-blue-500/5 m-1 rounded flex items-center justify-center relative">
                <span className="bg-black text-blue-500 px-1 rounded absolute top-0 right-0">Chunk 2</span>
            </div>
        </div>
...
const VisualGraphBuilding = () => (
    <div className="relative w-full h-64 flex items-center justify-center">
        <svg className="w-full h-full overflow-visible">
            {/* Central Node */}
            <circle cx="50%" cy="50%" r="8" fill="#fff" className="animate-ping absolute opacity-20" />
            <circle cx="50%" cy="50%" r="6" fill="#fff" />
            
            {/* Satellites */}
            <g className="animate-[spin_10s_linear_infinite] origin-center" style={{transformBox: 'fill-box'}}>
                <circle cx="50%" cy="20%" r="4" fill="#444" />
                <line x1="50%" y1="50%" x2="50%" y2="20%" stroke="#333" strokeDasharray="4" />
                
                <circle cx="80%" cy="50%" r="4" fill="#444" />
                <line x1="50%" y1="50%" x2="80%" y2="50%" stroke="#333" strokeDasharray="4" />
                
                <circle cx="20%" cy="80%" r="4" fill="#444" />
                <line x1="50%" y1="50%" x2="20%" y2="80%" stroke="#333" strokeDasharray="4" />
            </g>
        </svg>
    </div>
);
...
const VisualPageRankActive = () => (
    <div className="relative w-full h-64 flex items-center justify-center">
        ...
        <div className="relative w-64 h-64">
            ...
            {/* Small Nodes */}
            <div className="absolute top-10 left-10 w-8 h-8 bg-gray-700 rounded-full flex items-center justify-center text-[10px] text-gray-300">0.1</div>
            <div className="absolute bottom-10 right-20 w-12 h-12 bg-gray-600 rounded-full flex items-center justify-center text-[10px] text-gray-300">0.3</div>
            <div className="absolute top-20 right-10 w-6 h-6 bg-gray-800 rounded-full flex items-center justify-center text-[8px] text-gray-500">0.05</div>
            
            {/* Connections */}
            <svg className="absolute inset-0 w-full h-full pointer-events-none -z-10">
                <line x1="50%" y1="50%" x2="20%" y2="20%" stroke="#333" strokeWidth="2" />
                <line x1="50%" y1="50%" x2="70%" y2="80%" stroke="#333" strokeWidth="4" className="stroke-claude-ish/30" />
                <line x1="50%" y1="50%" x2="80%" y2="30%" stroke="#333" strokeWidth="1" />
            </svg>
        </div>
    </div>
);
        ...
        <div className="flex items-center gap-4 w-full">
            ...
            <div className="flex-1 bg-[#111] p-3 rounded border border-white/10 h-20 relative overflow-hidden">
                <div className="text-[10px] text-gray-400 font-mono">
                    <TypingAnimation text="Analyzing intent: This function handles JWT validation and expiry checks..." speed={30} />
                </div>
            </div>
        </div>
        <div className="flex gap-2">
            <span className="text-[10px] bg-gray-800 px-2 py-1 rounded text-gray-400">Context: 4k</span>
            <span className="text-[10px] bg-purple-900/30 text-purple-400 px-2 py-1 rounded">Model: Haiku</span>
        </div>
    </div>
);
...
const VisualQueryVec = () => (
    <div className="space-y-4 w-full max-w-sm">
        <div className="flex items-center gap-2">
            ...
            <div className="bg-[#1a1a1a] px-3 py-1 rounded text-white text-sm border border-white/10">"Where is auth?"</div>
        </div>
...
const VisualHybridSearch = () => (
    <div className="grid grid-cols-3 gap-2 w-full max-w-md">
        <div className="bg-[#111] border border-blue-500/30 p-3 rounded flex flex-col items-center gap-2">
            ...
            <div className="w-full bg-gray-800 h-1 rounded"><div className="bg-blue-500 w-[80%] h-full"></div></div>
        </div>
        <div className="bg-[#111] border border-green-500/30 p-3 rounded flex flex-col items-center gap-2">
            ...
            <div className="w-full bg-gray-800 h-1 rounded"><div className="bg-green-500 w-[40%] h-full"></div></div>
        </div>
        <div className="bg-[#111] border border-purple-500/30 p-3 rounded flex flex-col items-center gap-2">
            ...
            <div className="w-full bg-gray-800 h-1 rounded"><div className="bg-purple-500 w-[60%] h-full"></div></div>
        </div>
    </div>
);
...
const VisualScoreFusion = () => (
    <div className="flex flex-col gap-2 w-full max-w-xs">
        ...
        <div className="flex items-center gap-2 scale-110 transition-transform bg-[#1a1a1a] p-2 rounded border border-claude-ish/50 shadow-lg">
            <div className="w-8 h-8 bg-claude-ish rounded flex items-center justify-center text-black font-bold text-xs">#1</div>
...
const VisualAssembly = () => (
    <div className="font-mono text-[10px] text-gray-400 bg-[#111] p-4 rounded border border-gray-800 w-64 shadow-lg">
        ...
        <div className="pl-4"><span className="text-blue-400">"file"</span>: "src/auth.ts",</div>
        <div className="pl-4"><span className="text-blue-400">"rank"</span>: 0.94,</div>
        <div className="pl-4"><span className="text-blue-400">"context"</span>: "...",</div>
        <div className="pl-4"><span className="text-blue-400">"summary"</span>: "Validates..."</div>
        <div className="text-purple-400">{'}'}</div>
    </div>
);
