...
const HeroSection: React.FC<HeroSectionProps> = ({ onNavigateToBenchmarks, onNavigateToDocs }) => {
  const [rotation, setRotation] = useState({ x: 0, y: 0 });
  const [visibleLines, setVisibleLines] = useState<number>(0);
  const [status, setStatus] = useState({ model: 'Opus 4.5', cost: '$0.00', context: '1.2k' });
    ...
    const runSequence = () => {
      ...
      HERO_SEQUENCE.forEach((line, index) => {
        timeouts.push(setTimeout(() => {
            ...
            if (line.type === 'tool') {
                setStatus(prev => ({ ...prev, cost: '$0.02', context: '14.5k' }));
            }
        }, line.delay));
      });
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-start pt-20 pb-24 px-4 overflow-hidden">
        ...
        <div className="absolute top-0 left-0 w-full h-full -z-10 pointer-events-none">
            ...
            <div className="absolute top-[10%] right-[20%] w-[500px] h-[500px] bg-claude-ish/5 rounded-full blur-[100px] animate-pulse" style={{ animationDelay: '1s' }} />
        </div>
        ...
        <div className="text-center mb-12 max-w-5xl mx-auto z-10 flex flex-col items-center w-full">
            ...
            <h1 className="text-4xl md:text-7xl font-sans font-black tracking-tighter text-white mb-8 leading-[0.95] drop-shadow-2xl">
                ...
                Code Search <br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-claude-ish to-[#4fffa7]">for AI Agents</span>
            </h1>
        ...
        <div ref={containerRef} className="perspective-container w-full max-w-5xl relative h-[550px] group/terminal mt-24 opacity-80 hover:opacity-100 transition-opacity" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            <div className="w-full h-full transition-transform duration-300 ease-out preserve-3d" style={{ transform: `rotateX(${rotation.x}deg) rotateY(${rotation.y}deg)` }}>
                <TerminalWindow 
                                                ...
                                                <div className="flex-1 border-r border-[#30363d] pr-4 md:pr-6 flex items-center justify-center">
                                                    <div className="flex items-center gap-4 md:gap-6">
                                                        ...
                                                        <div className="flex flex-col text-left space-y-0.5 md:space-y-1">
                                                            ...
                                                            <div className="text-xs text-gray-600">~/dev/claudemem-landing</div>
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    {line.type === 'rich-input' && (
                                        <div className="mt-4 mb-2 px-2">
                                            <div className="flex items-start text-white group">
                                                ...
                                                <TypingAnimation text={line.content} speed={25} className="text-gray-100 font-medium" />
                                            </div>
                                        </div>
                                    )}
