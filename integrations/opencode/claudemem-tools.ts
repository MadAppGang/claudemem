...
PageRank > 0.05 = core abstraction, 0.01-0.05 = important, < 0.01 = utility`,
        args: {
          query: tool.schema.string().optional().describe("Focus area (optional, e.g., 'authentication')"),
          tokens: tool.schema.number().optional().describe("Max output tokens (default: 2000)"),
        },
...
Use when you know the symbol name and need its location.`,
        args: {
          name: tool.schema.string().describe("Symbol name to find (e.g., 'UserService', 'processPayment')"),
        },
        async execute({ name }) {
          try {
...
Use for: refactoring safety, understanding usage patterns`,
        args: {
          name: tool.schema.string().describe("Symbol name to find callers for"),
          limit: tool.schema.number().optional().describe("Max callers to show (default: 10)"),
        },
        async execute({ name, limit = 10 }) {
          try {
            const result = await $`claudemem --nologo callers ${name} --raw --callers ${limit}`
            if (result.exitCode !== 0) {
...
Use for: understanding implementation, tracing execution flow`,
        args: {
          name: tool.schema.string().describe("Symbol name to find callees for"),
          limit: tool.schema.number().optional().describe("Max callees to show (default: 15)"),
        },
        async execute({ name, limit = 15 }) {
          try {
            const result = await $`claudemem --nologo callees ${name} --raw --callees ${limit}`
            if (result.exitCode !== 0) {
...
Note: Exported symbols may be used externally.`,
        args: {
          includeExported: tool.schema.boolean().optional().describe("Include exported symbols (default: false)"),
          limit: tool.schema.number().optional().describe("Max results (default: 50)"),
        },
        async execute({ includeExported = false, limit = 50 }) {
          try {
            const exportedFlag = includeExported ? "--include-exported" : ""
            const result = await $`claudemem --nologo dead-code ${exportedFlag} -n ${limit} --raw`
            if (result.exitCode !== 0) {
...
Use for: test planning, QA prioritization.`,
        args: {
          minPagerank: tool.schema.number().optional().describe("Minimum PageRank threshold (default: 0.01)"),
          limit: tool.schema.number().optional().describe("Max results (default: 30)"),
        },
        async execute({ minPagerank = 0.01, limit = 30 }) {
          try {
            const result = await $`claudemem --nologo test-gaps --min-pagerank ${minPagerank} -n ${limit} --raw`
            if (result.exitCode !== 0) {
